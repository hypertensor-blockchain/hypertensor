...
use frame_system::{ pallet_prelude::{ BlockNumberFor, OriginFor }, ensure_signed, ensure_root };
use frame_support::{
...
// use hypertensor_node_runtime::BLOCKS_PER_HALVING;
...
pub mod pallet {
    ...
    pub type ValidatorRewardPercent<T> = StorageValue<
        _,
        u32,
        ValueQuery,
        DefaultValidatorRewardPercent<T>
    >;

    /// Pallet rewards for issuing rewards to block producers.
    #[pallet::pallet]
    #[pallet::without_storage_info]
    /// for testing purpses remove in production
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_validator_reward_percent(origin: OriginFor<T>, percent: u32) -> DispatchResult {
            ...
            ensure!(percent <= (100 as u32), Error::<T>::ValidatorPercentTooHigh);
...
impl<T: Config> Pallet<T> {
    fn do_initialize(_n: BlockNumberFor<T>) {
        ...
        let validator_percent = Perbill::from_rational(
            ValidatorRewardPercent::<T>::get(),
            100 as u32
        );
