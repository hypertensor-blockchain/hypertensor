...
// Executed Command:
// ./target/release/hypertensor-node benchmark pallet --chain=dev --wasm-execution=compiled --pallet=pallet_model_voting --extrinsic=* --steps=50 --repeat=20 --output="pallets/model-voting/src/weights.rs" --template ./.maintain/frame-weight-template.hbs
...
const DEFAUT_VOTE_AMOUNT: u128 = 1000e18 as u128;
...
fn build_model_peers<T: Config>(
    ...
    for n in start..end {
        let _ = T::Currency::deposit_creating(
            &funded_account::<T>("voter", n),
            deposit_amount.try_into().ok().expect("REASON")
        );
...
fn post_proposal_concluded<T: Config>(proposal_index: u32, proposer: T::AccountId) {
    ...
    assert_err!(
        ModelVoting::<T>::execute(RawOrigin::Signed(proposer.clone()).into(), proposal_index),
        Error::<T>::Concluded
    );
...
fn post_success_proposal_activate_ensures<T: Config>(
    path: Vec<u8>,
    proposal_index: u32,
    ...
    assert_eq!(
        proposal.max_block,
        proposal_start_block + block_to_u64::<T>(T::VotingPeriod::get())
    );
...
fn post_activate_cancel_ensures<T: Config>(
    proposal_index: u32,
    proposer: T::AccountId,
    path: Vec<u8>
) {
...
fn build_propose_activate<T: Config>(
    path: Vec<u8>,
    start: u32,
    end: u32,
    deposit_amount: u128
