...
impl<T: Config> Pallet<T> {
    pub fn do_add_delegate_stake(
        ...
        ensure!(delegate_stake_as_balance.is_some(), Error::<T>::CouldNotConvertToBalance);

        let account_delegate_stake_shares: u128 = AccountModelDelegateStakeShares::<T>::get(
            &account_id,
            model_id.clone()
        );
        let total_model_delegated_stake_shares = TotalModelDelegateStakeShares::<T>::get(
            model_id.clone()
        );
        let total_model_delegated_stake_balance = TotalModelDelegateStakeBalance::<T>::get(
            model_id.clone()
        );
        ...
        // );
            ...
            account_delegate_stake_balance.saturating_add(delegate_stake_to_be_added) <=
                MaxDelegateStakeBalance::<T>::get(),
        ...
        // --- Ensure the remove operation from the account_id is a success.
            ...
            Self::remove_balance_from_coldkey_account(
                &account_id,
                delegate_stake_as_balance.unwrap()
            ) == true,
        ...
        ensure!(delegate_stake_to_be_added_as_shares != 0, Error::<T>::CouldNotConvertToShares);
        ...
        Self::deposit_event(
            Event::DelegateStakeAdded(model_id, account_id, delegate_stake_to_be_added)
        );
    ...
    pub fn do_remove_delegate_stake(
        ...
        ensure!(delegate_stake_shares_to_be_removed > 0, Error::<T>::NotEnoughStakeToWithdraw);
        ...
        let delegate_stake_to_be_added_as_currency = Self::u128_to_balance(
        ...
        Self::add_balance_to_coldkey_account(
            &account_id,
            delegate_stake_to_be_added_as_currency.unwrap()
        );
        ...
        Self::deposit_event(
            Event::DelegateStakeRemoved(model_id, account_id, delegate_stake_to_be_removed)
        );
