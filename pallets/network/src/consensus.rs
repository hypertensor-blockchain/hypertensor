...
impl<T: Config> Pallet<T> {
    pub fn form_peer_consensus(block: u64) {
        ...
        let model_peer_consensus_submit_percent_requirement: u128 =
            ModelPeerConsensusSubmitPercentRequirement::<T>::get();
        ...
        let model_peer_seq_consensus_sent_threshold =
            ModelPeerConsecutiveConsensusSentThreshold::<T>::get();
        ...
        let peer_against_consensus_removal_threshold =
            PeerAgainstConsensusRemovalThreshold::<T>::get();
        let max_model_consensus_unconfirmed_seq_epochs =
            MaxModelConsensusUnconfirmedConsecutiveEpochs::<T>::get();
        ...
        for (model_id, data) in ModelsData::<T>::iter() {
            ...
            let model_consensus_epoch_errors: u32 = ModelConsensusEpochsErrors::<T>::get(
                model_id.clone()
            );
            if model_consensus_epoch_errors > max_model_consensus_epoch_errors {
                log::info!(
                    "Model ID {:?} surpassed maximum model consensus epoch errors",
                    model_id.clone()
                );
            ...
            if
                Self::percent_div(model_peer_submits as u128, total_model_peers as u128) <
                model_peer_consensus_submit_percent_requirement
            {
            ...
            let unconfirmed_count: u32 = ModelConsensusEpochUnconfirmedCount::<T>::get(
                model_id.clone()
            );
            let model_consensus_unconfirmed_seq_epochs_count =
                ModelConsensusUnconfirmedConsecutiveEpochsCount::<T>::get(model_id.clone());
            if
                Self::percent_div(unconfirmed_count as u128, model_peer_submits as u128) >=
                model_consensus_unconfirmed_threshold
            {
                log::info!(
                    "Model ID {:?} consensus data unconfirmed by {:?} model peers",
                    model_id.clone(),
                    unconfirmed_count
                );
                // Increase the count of unconfirmed epochs in a row
                // This resets every successful epoch
                ModelConsensusUnconfirmedConsecutiveEpochsCount::<T>::mutate(
                    model_id.clone(),
                    |n: &mut u32| {
                        *n += 1;
                    }
                ...
                if
                    model_consensus_unconfirmed_seq_epochs_count + 1 >
                    max_model_consensus_unconfirmed_seq_epochs
                {
                    ModelConsensusEpochsErrors::<T>::mutate(model_id.clone(), |n: &mut u32| {
                        *n += 1;
                    });
            ...
            let model_peer_submit_submissions = ModelConsensusEpochSubmitCount::<T>::get(
                model_id.clone()
            );
            ...
            if
                Self::percent_div(
                    model_peer_submit_submissions as u128,
                    total_model_peers as u128
                ) < model_peer_consensus_submit_percent_requirement
            {
            ...
            let model_consecutive_successful_epochs = ModelConsecutiveSuccessfulEpochs::<T>::get(
                model_id.clone()
            );
            if
                model_consecutive_successful_epochs >= model_consecutive_epochs_threshold &&
                model_consecutive_successful_epochs % model_consecutive_epochs_threshold == 0
            {
            ...
            for peer_consensus_result in ModelPeerConsensusResults::<T>::iter_prefix_values(
                model_id.clone()
            ) {
                ...
                let model_peer_exists: bool = ModelPeersData::<T>::contains_key(
                    model_id.clone(),
                ...
                if !model_peer_exists {
                    ModelPeerConsensusResults::<T>::remove(
                        model_id.clone(),
                        consensus_result_account_id.clone()
                    );
                    continue;
                ...
                if removal_consensus_percentage > peer_removal_threshold {
                    ...
                    Self::do_remove_model_peer(
                        block,
                        model_id.clone(),
                        consensus_result_account_id.clone()
                    );

                    let consensus_result_successful_consensus: Vec<T::AccountId> =
                        peer_consensus_result.successful_consensus;
                    ...
                    for dishonest_account_id in consensus_result_successful_consensus.iter() {
                        if
                            let Some(count) = against_consensus_peer_count.get_mut(
                                &dishonest_account_id.clone()
                            )
                        {
                            *count += 1;
                        } else {
                    ...
                    //
                    let peer_submitted: bool = PeerConsensusEpochSubmitted::<T>::get(
                        model_id.clone(),
                        consensus_result_account_id.clone()
                    );

                    let peer_unconfirmed: bool = PeerConsensusEpochUnconfirmed::<T>::get(
                        model_id.clone(),
                        consensus_result_account_id.clone()
                    );
                    ...
                    let consensus_result_unsuccessful_consensus: Vec<T::AccountId> =
                        peer_consensus_result.unsuccessful_consensus;
                    ...
                    for dishonest_account_id in consensus_result_unsuccessful_consensus.iter() {
                        ...
                        AccountPenaltyCount::<T>::mutate(dishonest_account_id, |n: &mut u32| {
                            *n += 1;
                        });
                    }
                }
            ...
            for peer_consensus_result in ModelPeerConsensusResults::<T>::iter_prefix_values(
                ...
                let delta: u128 = (((avg_score as f64) * (maximum_outlier_delta_percent as f64)) /
                    100.0) as u128;
                ...
                for score in consensus_result_scores.iter() {
                    ...
                    if *score <= min_required_score || *score >= max_required_score {
                        let account_id: &T::AccountId = &consensus_result_successful_consensus[
                            score_index
                        ];
                        ...
                        AccountPenaltyCount::<T>::mutate(account_id, |n: &mut u32| {
                            *n += 1;
                        });
                    }
                    score_index += 1;
            ...
            for model_peer in ModelPeersData::<T>::iter_prefix_values(model_id.clone()) {
                ...
                if let Some(count) = against_consensus_peer_count.get_mut(&account_id.clone()) {
                    let against_percent: u128 = Self::percent_div(
                        *count as u128,
                        consensus_peer_count as u128
                    );
                ...
                if !peer_submitted && !peer_unconfirmed && can_submit {
                    ...
                    ModelPeerConsecutiveConsensusSent::<T>::insert(
                        model_id.clone(),
                        account_id.clone(),
                        0
                    );
                    ModelPeerConsecutiveConsensusNotSent::<T>::mutate(
                        model_id.clone(),
                        account_id.clone(),
                        |n: &mut u32| {
                            *n += 1;
                    ...
                    AccountPenaltyCount::<T>::mutate(account_id.clone(), |n: &mut u32| {
                        *n += 1;
                    });
                } else {
                    // Increase consensus sent sequence count
                    ModelPeerConsecutiveConsensusSent::<T>::mutate(
                        model_id.clone(),
                        account_id.clone(),
                        |n: &mut u32| {
                            *n += 1;
                    ...
                    //
                    if
                        model_peer_seq_consensus_sent >= model_peer_seq_consensus_sent_threshold &&
                        model_peer_seq_consensus_sent % model_peer_seq_consensus_sent_threshold == 0
                    {
                    ...
                    let model_peer_seq_consensus_not_sent =
                        ModelPeerConsecutiveConsensusNotSent::<T>::get(
                            model_id.clone(),
                            account_id.clone()
                        );
                    ...
                    if model_peer_seq_consensus_not_sent > 0 {
                        ModelPeerConsecutiveConsensusNotSent::<T>::insert(
                            model_id.clone(),
                            account_id.clone(),
                            0
                        );
                ...
                PeerConsensusEpochSubmitted::<T>::insert(
                    model_id.clone(),
                    account_id.clone(),
                    false
                );
            ...
            ModelsInConsensus::<T>::append(model_id.clone());
            ModelConsecutiveSuccessfulEpochs::<T>::mutate(model_id.clone(), |n: &mut u32| {
                *n += 1;
            });
        }
    }
