...
use crate::{
    ...
    MaxAccountPenaltyCount,
    MinModelPeers,
    ModelConsensusUnconfirmedThreshold,
    ModelPeersData,
    ModelPeerAccount,
    ModelAccount,
    ModelConsensusEpochsErrors,
    ...
    AccountModelStake,
    MinStakeBalance,
    ModelTotalConsensusSubmits,
    PeerAgainstConsensusRemovalThreshold,
    ...
    DishonestyVotingPeriod,
    ModelPeerDishonestyVote,
    MinRequiredPeerConsensusDishonestyEpochs,
    AccountModelDelegateStakeShares,
    TotalModelDelegateStakeShares,
    TotalModelDelegateStakeBalance,
...
fn build_model(model_path: Vec<u8>) {
    assert_ok!(Network::vote_model(RuntimeOrigin::signed(account(0)), model_path.clone()));

    assert_ok!(Network::add_model(RuntimeOrigin::signed(account(0)), model_path.clone()));
}

// Returns total staked on model
fn build_model_peers(
    model_id: u32,
    start: u32,
    end: u32,
    deposit_amount: u128,
...
fn build_for_submit_consensus_data(
    model_id: u32,
    start: u32,
    end: u32,
    start_data: u32,
...
fn make_model_submittable() {
    ...
    let min_required_model_consensus_submit_epochs: u64 =
        Network::min_required_model_consensus_submit_epochs();
    System::set_block_number(
        System::block_number() +
            consensus_blocks_interval * min_required_model_consensus_submit_epochs
    );
}
...
fn make_consensus_data_submittable() {
    ...
    let start_block_can_remove_peer =
        ((consensus_blocks_interval as u128) * model_peer_removal_percentage) / PERCENTAGE_FACTOR;

    let max_remove_model_peer_block =
        (start_block_can_remove_peer as u64) +
        (current_block_number - (current_block_number % consensus_blocks_interval));
...
fn make_model_peer_included() {
    ...
    let min_required_consensus_inclusion_epochs =
        MinRequiredPeerConsensusInclusionEpochs::<Test>::get();
    System::set_block_number(
        System::block_number() + consensus_blocks_interval * min_required_consensus_inclusion_epochs
    );
}
...
fn make_model_peer_consensus_data_submittable() {
    ...
    let min_required_peer_consensus_submit_epochs: u64 =
        Network::min_required_peer_consensus_submit_epochs();
    System::set_block_number(
        System::block_number() +
            consensus_blocks_interval * min_required_peer_consensus_submit_epochs
    );
...
fn make_model_peer_dishonesty_consensus_proposable() {
    ...
    let consensus_blocks_interval = ConsensusBlocksInterval::<Test>::get();
    let min_required_peer_consensus_submit_epochs: u64 =
        MinRequiredPeerConsensusDishonestyEpochs::<Test>::get();
    System::set_block_number(
        System::block_number() +
            consensus_blocks_interval * min_required_peer_consensus_submit_epochs
    );
...
fn make_model_peer_removable() {
    ...
    let block_span_can_remove_peer = (((consensus_blocks_interval as u128) *
        model_peer_removal_percentage) /
        PERCENTAGE_FACTOR) as u64;

    let start_removal_block = (CONSENSUS_STEPS +
        (current_block_number - (current_block_number % consensus_blocks_interval))) as u64;

    let end_removal_block =
        block_span_can_remove_peer +
        (current_block_number - (current_block_number % consensus_blocks_interval));
...
fn post_successful_add_model_peer_asserts(n: u32, model_id: u32, amount: u128) {
...
fn post_successful_add_model_peers_asserts(total_peers: u32, stake_per_peer: u128, model_id: u32) {
...
fn post_remove_model_peer_ensures(n: u32, model_id: u32) {
    ...
    let model_peer_consensus_results = ModelPeerConsensusResults::<Test>::try_get(
        model_id,
        account(n)
    );
...
fn post_successful_form_consensus_ensures(model_id: u32) {
    ...
    let model_consensus_epoch_unconfirmed_count =
        ModelConsensusEpochUnconfirmedCount::<Test>::try_get(model_id.clone());
...
fn post_successful_dishonesty_proposal_ensures(proposer: u32, votee: u32, model_id: u32) {
    let model_peer_dishonesty_vote = ModelPeerDishonestyVote::<Test>::get(
        model_id.clone(),
        account(votee)
    );
...
fn test_add_model_err() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Network::add_model(RuntimeOrigin::signed(account(0)), model_path.clone()),
...
fn test_remove_model() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Network::vote_model_out(RuntimeOrigin::signed(account(0)), model_path.clone()));

        let model_id = ModelPaths::<Test>::get(model_path.clone()).unwrap();

        assert_ok!(Network::remove_model(RuntimeOrigin::signed(account(0)), model_id));
...
fn test_remove_model_err() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Network::remove_model(RuntimeOrigin::signed(account(0)), 255),
...
fn test_add_model_max_models_err() {
    new_test_ext().execute_with(|| {
        ...
        for m in 0..n_models {
            ...
            assert_ok!(
                Network::vote_model(RuntimeOrigin::signed(account(0)), model_path.clone().into())
            );

            if m + 1 < n_models {
                assert_ok!(
                    Network::add_model(RuntimeOrigin::signed(account(0)), model_path.clone().into())
                );
            } else {
                assert_err!(
                    Network::add_model(
    ...
    new_test_ext().execute_with(|| {
        ...
        for n in 0..n_peers {
            ...
            if n + 1 < n_peers {
                total_staked += amount;
                assert_ok!(
                    add_model_peer(n, model_id.clone(), n, "172.20.54.234".into(), 8888, amount)
                );
...
fn test_add_model_peer_remove_readd_err() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Network::remove_model_peer(RuntimeOrigin::signed(account(0)), model_id.clone()));
...
fn test_add_model_peer_remove_readd() {
    new_test_ext().execute_with(|| {
        ...
        System::set_block_number(
            System::block_number() + consensus_blocks_interval * min_required_unstake_epochs
        );
...
fn test_add_model_peer_remove_stake_partial_readd() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(
            Network::remove_stake(RuntimeOrigin::signed(account(0)), model_id.clone(), amount)
        );
...
fn test_add_model_peer_remove_stake_readd() {
    new_test_ext().execute_with(|| {
        ...
        let remaining_account_stake_balance: u128 = AccountModelStake::<Test>::get(
            &account(0),
            model_id.clone()
        );
...
fn test_update_model_peer_peer_id_existing_err() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Network::update_model_peer(RuntimeOrigin::signed(account(0)), model_id, peer(0)),
            Error::<Test>::PeerIdExist
        );
    });
}
...
fn test_update_model_peer_invalid_epoch_err() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Network::update_model_peer(RuntimeOrigin::signed(account(0)), model_id, peer(1)),
...
fn test_update_port_err() {
    new_test_ext().execute_with(|| {
        ...
        // invalid account
        assert_err!(
            Network::update_port(RuntimeOrigin::signed(account(255)), model_id.clone(), 65535),
            Error::<Test>::ModelPeerNotExist
        );

        // invalid model
        assert_err!(
            Network::update_port(RuntimeOrigin::signed(account(0)), 255, 8889),
            Error::<Test>::ModelNotExist
        );
    })
}
...
fn test_update_port() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(
            Network::update_port(RuntimeOrigin::signed(account(0)), model_id.clone(), 65535)
        );
    })
}
...
fn test_submit_consensus_min_required_model_epochs() {
    new_test_ext().execute_with(|| {
        ...
        post_successful_add_model_peers_asserts(n_peers.into(), amount, model_id.clone());
    ...
    new_test_ext().execute_with(|| {
        ...
        System::set_block_number(
            consensus_blocks_interval +
                (System::block_number() - (System::block_number() % consensus_blocks_interval))
        );

        assert_ok!(Network::form_consensus(RuntimeOrigin::signed(account(0))));
...
fn test_submit_consensus_data_dishonest() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(len, n_peers as usize, "ModelPeerConsensusResults len mismatch.");
...
fn test_submit_consensus_data_remove_peer() {
    new_test_ext().execute_with(|| {
        ...
        let model_peer_consensus_submit_percent_requirement =
            Network::model_peer_consensus_submit_percent_requirement();

        let n_peers: u32 = Network::max_model_peers();
        // Get amount of peers that need to keep a peer absent so they are removed through consensus
        let n_consensus_peers: u32 = (
            (n_peers as f64) *
            ((model_peer_consensus_submit_percent_requirement as f64) / 10000.0)
        ).ceil() as u32;
        ...
        build_for_submit_consensus_data(
            model_id.clone(),
            0,
            n_consensus_peers,
            0,
        ...
        assert_eq!(
            Network::model_peer_consensus_results(1, account(0)).successful_consensus.len(),
            n_consensus_peers as usize
        );
        assert_eq!(
            Network::model_peer_consensus_results(1, account(0)).successful,
            n_consensus_peers as u32
        );
        assert_eq!(
            Network::model_peer_consensus_results(1, account(0)).unsuccessful_consensus.len(),
            0 as usize
        );
        assert_eq!(Network::model_peer_consensus_results(1, account(0)).unsuccessful, 0 as u32);
        assert_eq!(
            Network::model_peer_consensus_results(1, account(0)).total_submits,
            n_consensus_peers as u32
        );
        ...
        assert_eq!(
            Network::model_peer_consensus_results(
                1,
                account(n_consensus_peers)
            ).unsuccessful_consensus.len(),
            n_consensus_peers as usize
        );
        assert_eq!(
            Network::model_peer_consensus_results(1, account(n_consensus_peers)).unsuccessful,
            n_consensus_peers as u32
        );
        assert_eq!(
            Network::model_peer_consensus_results(1, account(n_consensus_peers)).account_id,
            account(n_consensus_peers)
        );
        assert_eq!(
            Network::model_peer_consensus_results(1, account(n_consensus_peers)).peer_id,
            peer(n_consensus_peers)
        );
...
fn test_generate_emissions() {
    new_test_ext().execute_with(|| {
        ...
        StakeVaultBalance::<Test>::mutate(|n: &mut u128| {
            *n += 4000000000000000000000;
        });
        ...
        System::set_block_number(
            consensus_blocks_interval +
                (System::block_number() - (System::block_number() % consensus_blocks_interval) + 1)
        );

        assert_ok!(Network::do_generate_emissions(RuntimeOrigin::signed(account(0))));
        ...
        let submissions = ModelPeerConsensusResults::<Test>::iter_key_prefix(model_id.clone());
        let len = submissions.count();
        assert_eq!(len, 0, "ModelPeerConsensusResults len mismatch.");
        ...
        assert!(
            post_total_vault_balance <= expected_max_post_vault_balance,
            "post_total_vault_balance {:?} expected_max_post_vault_balance {:?}",
            post_total_vault_balance,
            expected_max_post_vault_balance
        );
...
fn test_remove_peer_err() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Network::remove_model_peer(RuntimeOrigin::signed(account(255)), 0),
            Error::<Test>::ModelNotExist
        );
        ...
        post_successful_add_model_peers_asserts(1, amount, model_id.clone());

        assert_eq!(Network::total_stake(), amount);

        assert_err!(
            Network::remove_model_peer(RuntimeOrigin::signed(account(255)), model_id.clone()),
            Error::<Test>::ModelPeerNotExist
        );
...
fn test_remove_peer_is_included_err() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Network::remove_model_peer(RuntimeOrigin::signed(account(0)), model_id.clone()),
            Error::<Test>::InvalidRemoveOrUpdateModelPeerBlock
        );
...
fn test_add_to_stake_err() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Network::add_to_stake(RuntimeOrigin::signed(account(0)), 0, amount),
            Error::<Test>::ModelNotExist
        );
        ...
        assert_err!(
            Network::add_to_stake(RuntimeOrigin::signed(account(255)), model_id.clone(), amount),
            Error::<Test>::ModelPeerNotExist
        );
    });
}
...
fn test_add_to_stake() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(
            Network::add_to_stake(RuntimeOrigin::signed(account(0)), model_id.clone(), amount)
        );
...
fn test_remove_stake_err() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Network::remove_stake(RuntimeOrigin::signed(account(255)), 0, amount),
            Error::<Test>::ModelPeerNotExist
        );
        ...
        assert_err!(
            Network::remove_stake(RuntimeOrigin::signed(account(255)), model_id.clone(), amount),
            Error::<Test>::ModelPeerNotExist
        );

        assert_err!(
            Network::remove_stake(RuntimeOrigin::signed(account(0)), model_id.clone(), 0),
            Error::<Test>::RequiredUnstakeEpochsNotMet
        );
        ...
        assert_err!(
            Network::remove_stake(RuntimeOrigin::signed(account(0)), model_id.clone(), amount + 1),
            Error::<Test>::NotEnoughStakeToWithdraw
        );
...
fn test_form_consensus_unconfirm_consensus() {
    new_test_ext().execute_with(|| {
        ...
        let n_peers_unconfirm: u32 = (
            (n_peers as f64) *
            ((unconfirm_threshold as f64) / 10000.0)
        ).ceil() as u32;
        ...
        let amount_staked = build_model_peers(
            model_id.clone(),
            0,
            n_peers,
        ...
        for n in 0..n_peers_unconfirm {
            assert_ok!(
                Network::unconfirm_consensus_data(RuntimeOrigin::signed(account(n)), model_id)
            );
        }
        ...
        let max_model_model_unconfirmed_epochs =
            MaxModelConsensusUnconfirmedConsecutiveEpochs::<Test>::get();

        // Should have increased uncofirmed epochs count
        let model_uncofirmed_epochs = ModelConsensusUnconfirmedConsecutiveEpochsCount::<Test>::get(
            model_id.clone()
        );
        assert_eq!(
            model_uncofirmed_epochs,
            1,
            "ModelConsensusUnconfirmedConsecutiveEpochsCount incorrect."
        );
...
fn test_submit_data_consensus_as_err_then_unconfirm_consensus_err() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Network::unconfirm_consensus_data(RuntimeOrigin::signed(account(0)), model_id));

        // cannot call `unconfirm_consensus_data` if already submitted consensus data with an error state
        assert_err!(
            Network::unconfirm_consensus_data(RuntimeOrigin::signed(account(0)), model_id),
...
fn test_form_consensus() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Network::model_peer_consensus_results(1, account(n_peers - 1)).unsuccessful,
            0 as u32
        );
        assert_eq!(
            Network::model_peer_consensus_results(1, account(n_peers - 1)).account_id,
            account(n_peers - 1)
        );
        assert_eq!(
            Network::model_peer_consensus_results(1, account(n_peers - 1)).peer_id,
            peer(n_peers - 1)
        );
...
fn test_form_consensus_remove_peer() {
    new_test_ext().execute_with(|| {
        ...
        let n_peers: u32 = Network::max_model_peers();
        let n_required_against_peers: u32 =
            (((n_peers as f64) * ((peer_removal_threshold as f64) / 10000.0)).ceil() as u32) + 1;
...
fn test_percent_mul() {
    new_test_ext().execute_with(|| {
        ...
        let value = Network::percent_mul(28000000e18 as u128, PERCENTAGE_FACTOR);

        assert_ne!(value, 0, "percent_mul didn't round down");
        assert_ne!(value, u128::MAX, "percent_mul didn't round down");

        let value = Network::percent_mul_round_up(28000000e18 as u128, PERCENTAGE_FACTOR);
...
fn test_submit_consensus_data_remove_peer_peer_against_consensus_removal_threshold() {
    new_test_ext().execute_with(|| {
        ...
        let model_peer_consensus_submit_percent_requirement =
            Network::model_peer_consensus_submit_percent_requirement();
        let peer_against_consensus_removal_threshold: u128 =
            PeerAgainstConsensusRemovalThreshold::<Test>::get();
        ...
        let n_peers_threshold = Network::percent_mul_round_up(
            n_peers as u128,
            peer_against_consensus_removal_threshold
        );
        ...
        assert_eq!(
            AccountPenaltyCount::<Test>::get(account(n_peers - 1)),
            n_peers_threshold as u32
        );
...
fn test_form_consensus_remove_ineligible_model_peer() {
    new_test_ext().execute_with(|| {
        ...
        for n in 0..max_account_penalty_count {
            let dishonest_peer_account_penalty_count = AccountPenaltyCount::<Test>::get(
                account(n_peers - 1)
            );
        ...
        assert!(
            AccountPenaltyCount::<Test>::get(account(n_peers - 1)) >= max_account_penalty_count
        );
    });
}
...
fn test_vote_model_peer_dishonest_model_exists_error() {
    new_test_ext().execute_with(|| {
        assert_err!(
            Network::vote_model_peer_dishonest(RuntimeOrigin::signed(account(0)), 1, peer(1)),
            Error::<Test>::ModelNotExist
        );
    });
}
...
fn test_vote_model_peer_dishonest_consensus() {
    new_test_ext().execute_with(|| {
        ...
        let n_peers: u32 = Network::max_model_peers();
        let n_required_voting_peers: u32 =
            (((n_peers as f64) * ((peer_removal_threshold as f64) / 10000.0)).ceil() as u32) + 1;
...
fn test_add_to_delegate_stake() {
    new_test_ext().execute_with(|| {
        ...
        let total_model_delegated_stake_shares = TotalModelDelegateStakeShares::<Test>::get(
            model_id.clone()
        );
        ...
        let delegate_shares = AccountModelDelegateStakeShares::<Test>::get(
            account(0),
            model_id.clone()
        );
...
fn test_add_to_delegate_stake_increase_pool_check_balance() {
    new_test_ext().execute_with(|| {
        ...
        Network::increase_delegated_stake(model_id.clone(), increase_delegated_stake_amount);
...
fn test_remove_to_delegate_stake() {
    new_test_ext().execute_with(|| {
        ...
        System::set_block_number(
            System::block_number() +
                consensus_blocks_interval * min_required_delegate_unstake_epochs
        );
