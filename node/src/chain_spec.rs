...
pub fn authority_keys_from_ss58(s_aura: &str, s_grandpa: &str) -> (AuraId, GrandpaId) {
    (aura_from_ss58_addr(s_aura), grandpa_from_ss58_addr(s_grandpa))
}
        ...
        ChainSpec::from_genesis(
            ...
            move || {
                testnet_genesis(
                    ...
                    // Pre-funded accounts
                    (0..90)
                        .map(|x| get_account_id_from_seed::<sr25519::Public>(&x.to_string()))
                        .collect::<Vec<_>>()
                    // vec![
                    // 	get_account_id_from_seed::<sr25519::Public>("Alice"),
...
// -> Sudo only
// ./target/release/hypertensor-node build-spec --disable-default-bootnode --chain vitalik > vitalikSpec.json
// ./target/release/hypertensor-node build-spec --chain=vitalikSpec.json --raw --disable-default-bootnode > vitalikSpecRaw.json
        ...
        balances: BalancesConfig {
            ...
            balances: {
                endowed_accounts
                    .iter()
                    .cloned()
                    .map(|k| (k, 10000000000000000000000))
                    .collect()
            },
        },
        aura: AuraConfig {
